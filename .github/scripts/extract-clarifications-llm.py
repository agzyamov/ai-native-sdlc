...
def extract_markers_with_llm(spec_content: str) -> list[dict]:
    ...
    try:
        ...
        for marker in markers:
            ...
            if missing_required:
                print(
                    f"⚠️  Warning: Marker missing required fields: {missing_required}",
                    file=sys.stderr,
                )
...
def generate_clarifications_md(
    markers: list[dict], feature_name: str, spec_link: str = "./spec.md"
) -> str:
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract clarification markers from spec.md or spec_output.txt using LLM"
    )
    ...
    parser.add_argument(
        "--output", required=True, help="Path to output JSON file with extracted questions"
    )
    parser.add_argument(
        "--feature-name",
        help="Feature name for header (optional, used only for clarifications.md generation)",
    )
    ...
    if args.spec_output_file:
        ...
        print(
            "❌ Error: Either --spec-file or --spec-output-file must be provided", file=sys.stderr
        )
        sys.exit(1)
