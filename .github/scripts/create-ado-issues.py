...
try:
    ...
    print(
        "❌ Error: Could not import ado_client. Make sure function_app is in the path.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def validate_and_fix_markdown(markdown_text: str) -> tuple[str, list[str]]:
    ...
    if (italic_count - italic_in_links * 2) % 2 != 0:
        warnings.append("Possible unclosed italic markers detected")
...
def build_description(
    ...
    raw_description_parts.append("**Your choice**: _[Awaiting response]_\n\n")
...
Return ONLY the cleaned and fixed markdown description, no code blocks, no explanation."""
            ...
            if not description or not description.strip():
                print("⚠️  LLM returned empty description, using raw description", file=sys.stderr)
                return raw_description

            # Check if LLM response has meaningful content (at least a header or some text)
            if len(description.strip()) < 10:
                print(
                    f"⚠️  LLM returned very short description ({len(description)} chars), using raw description",
                    file=sys.stderr,
                )
                return raw_description
            ...
            if "\n" not in description and "\n" in raw_description:
                print(
                    f"⚠️  LLM stripped all newlines (raw had {raw_description.count(chr(10))} newlines), using raw description",
                    file=sys.stderr,
                )
                return raw_description

            print("✅ Cleaned and fixed markdown using LLM")
            return description
...
Topic:"""
        title_response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Extract clean topic text. Remove 'Question N:' prefixes. Return only the topic.",
                },
...
def main():
    ...
    for i, q in enumerate(questions, 1):
        try:
            ...
            recommended_option = (
                q.get("recommended_option", "").strip() if q.get("recommended_option") else None
            )
            suggested_answer = (
                q.get("suggested_answer", "").strip() if q.get("suggested_answer") else None
            )
            ...
            if warnings:
                print("⚠️  Markdown validation warnings:")
            ...
            if not final_description or not final_description.strip():
                print(
                    f"❌ Error: Final description is empty for Question {i}, skipping issue creation",
                    file=sys.stderr,
                )
                print(
                    f"   Raw description length: {len(description) if 'description' in locals() else 'N/A'}",
                    file=sys.stderr,
                )
                continue
            ...
            if result:
                ...
                print(
                    f"⚠️ Issue creation failed for Question {i} (may be duplicate or API error)",
                    file=sys.stderr,
                )
