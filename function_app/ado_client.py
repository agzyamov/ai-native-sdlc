...
def get_work_item(work_item_id: int) -> dict | None:
    """
    ...
    if not all([org_url, project, pat]):
        logger.error(
            "Missing required ADO environment variables (ADO_ORG_URL, ADO_PROJECT, ADO_WORK_ITEM_PAT)"
        )
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to fetch work item {work_item_id}: HTTP {response.status_code} - {response.text[:500]}"
            )
            return None
        ...
        logger.error(f"Error fetching work item {work_item_id}: {e!s}")
        return None
...
def get_work_item_latest_revision(work_item_id: int) -> dict | None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to fetch revision {latest_rev} for work item {work_item_id}: HTTP {response.status_code} - {response.text[:500]}"
            )
            return None
        ...
        logger.error(f"Error fetching revision {latest_rev} for work item {work_item_id}: {e!s}")
        return None
...
def update_work_item_description(work_item_id: int, description: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Error updating work item {work_item_id}: {e!s}")
        return False
...
def create_issue_workitem(
    ...
    try:
        ...
        logger.error(f"Error creating Issue: {e!s}")
        return None
...
def get_child_issues(parent_feature_id: int) -> list[dict]:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"WIQL query failed for parent {parent_feature_id}: HTTP {response.status_code} - {response.text[:500]}"
            )
            return []
        ...
        logger.info(
            f"Found {len(work_item_ids)} closed Issues for Feature {parent_feature_id}: {work_item_ids}"
        )
        ...
        if batch_response.status_code != 200:
            logger.error(
                f"Batch fetch failed for Issues: HTTP {batch_response.status_code} - {batch_response.text[:500]}"
            )
            return []
        ...
        logger.error(f"Error fetching child Issues for Feature {parent_feature_id}: {e!s}")
        return []
...
def get_work_item_comments(work_item_id: int) -> list[str]:
    ...
    comments_url = (
        f"{org_url}/{project}/_apis/wit/workitems/{work_item_id}/comments?api-version=7.0-preview.3"
    )
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to fetch comments for work item {work_item_id}: HTTP {response.status_code} - {response.text[:500]}"
            )
            return []
        ...
        logger.error(f"Error fetching comments for work item {work_item_id}: {e!s}")
        return []
